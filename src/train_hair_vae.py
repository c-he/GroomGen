...
import dnnlib
from models import HairstyleVAE
from torch_utils import misc, training_stats
from training.dataset import NeuralTextureDataset
from training.loss import HairstyleVAELoss
...
def main(**kwargs):
    ...
    if rank == 0:
        ...
        model_string_name = 'hairstyle-vae'
    ...
    # Create model:
    model = HairstyleVAE(
        in_features=dataset.raw_channels + 1,
        out_features=dataset.raw_channels + 1,
    ...
    loss = HairstyleVAELoss(device=device, model=model, lambda_kl=opts.lambda_kl)
