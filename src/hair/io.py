...
def load_hair(path: str) -> np.ndarray:
    ...
    for _ in range(num_strands):
        ...
        points = struct.unpack(
            "f" * num_points * 3, data[idx + 4 : idx + 4 + 4 * num_points * 3]
        )
...
def save_hair(path: str, data: np.ndarray) -> None:
    ...
    assert ext in [
        ".data",
        ".obj",
    ], "only support saving hair data in .data and .obj format"
...
def _save_hair_obj(path: str, data: np.ndarray) -> None:
    ...
    edges = np.reshape(
        np.arange(data.shape[0] * data.shape[1]), [data.shape[0], data.shape[1]]
    )
    edges = edges + 1
